		...
		loadChanges: function () {
			...
			return new Promise(function (resolve, reject) {
				$.ajax({
					...
					function isNonMTAWithChanges(sPath) {
						return sPath.indexOf("webapp/changes/") === 0 && sPath.endsWith(".change");
					}

					function isMTAWithChanges(sPath) {
						...
						return sPath.startsWith("ZSPM_IDOC_APPR_RPT") && sPath.indexOf("webapp/changes/") > 0 && sPath.endsWith(".change");
					}

					//we are looking for only change files
					var aChangeFilesPaths = Object.keys(oCachebusterContent).filter(function (sPath) {
						return isNonMTAWithChanges(sPath) || isMTAWithChanges(sPath);
					});
					...
					return Promise.all(aPromises).then(function (aChanges) {
						...
						if (aChangePromises.length > 0) {
							return Promise.all(aChangePromises).then(function (aUpdatedChanges) {
								...
								var oLrepChange = {
									changes: oResult,
									componentClassName: "mmal.spm.fi.zspm_idoc_appr_rpt"
								};
								resolve(oLrepChange);
							});
						} else {
